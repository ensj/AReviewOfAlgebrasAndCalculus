...
Thankfully, this is a solved problem. It turns out that polynomials behave very, very nicely under basic arithmetic operations (we say they are \textit{closed} under addition and multiplation). They work fairly similarly to how we add, subtract, multiply and divide real numbers together, so this section shouldn't take long!

\subsection{Polynomial Addition}

First recall that variables of different powers cannot be added together. Consider the following expression.

\[
  x^2 + x^3
\]

There is no way to simplify the above expression further. Similarly, if the base (that is, the variables) of each term is different, we cannot add two terms together despite them having the same power. Let's take a look.
...
You can think of polynomial factoring as a 
